...
const Main = () => {
  const { scrollTo, homeRef, servicesRef, aboutRef, contactRef } =
  ...
  useEffect(() => {
    ...
    if (section) {
      ...
      setTimeout(() => {
        scrollTo(ref);
        window.history.replaceState({}, "", "/"); // clean the query param
      }, 100);
    }
  }, [searchParams]);
        ...
        <div className={styles.headline}>
          ...
          >
            <h2 className={poppins.className}>
              <span className={styles.gradient}>Designing</span> the World we
              envision through{" "}
              <span className={styles.gradient}>Engineering</span>
            </h2>
          </motion.div>
